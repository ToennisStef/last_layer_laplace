...
| M7 | "Laplace" is a *family*: `Sigma = (C + tau*I)^-1` with two independent axes — which curvature `C` (exact Hessian / GGN / Fisher / MC Fisher / empirical Fisher) and what structure it may have (dense / KFAC / diagonal). The structure axis buys scale; the curvature axis buys *existence* | read | [curvature-approximations](methods/curvature-approximations.md) |
