...
# Module level, not inside main(): with mp_context="spawn" every chain worker is a
# fresh interpreter that re-imports this module but never executes the __main__
# block, so a cap applied in main() would leave each worker back at its 16-thread
# default - mcmc_chains x 16 threads competing for 16 cores. Measured here: a
# 200x20 matmul (this project's shape) takes 4.4 us on one thread and 12332 us on
# sixteen, because the OpenMP barrier dwarfs the arithmetic. Equivalent to
# launching with OMP_NUM_THREADS=1, but does not rely on remembering the flag -
# the same argument as mp_context in stage 3.
# doc/gotchas/fork-deadlock-multichain-nuts.md
...
def run_arm(cfg: Config, spec: ArmSpec, arm_dir: Path, *, args) -> str:
    ...
    # disable_progbar: the per-chain tqdm bars go to *stderr*, so they never reached
    # run.log (Tee wraps stdout only) but they dominate any launch log that folds
    # stderr in with `2>&1` - mcmc_chains interleaved bars, rewritten in place with
    # ANSI cursor codes. The stage-3 line above and mcmc.summary() below are both
    # stdout, so silencing the bars loses nothing that is kept.
    mcmc = MCMC(
        kernel,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=int,
        default=None,
        help="override Config.mcmc_chains; each chain is a separate process holding "
