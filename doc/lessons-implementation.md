...
| I13 | Torch spreads every matmul over all cores. Two failure modes for multi-chain MCMC: **F1** each chain process claims all 16 cores, so `chains x 16` threads fight for 16 and mostly wait; **F2** the per-op coordination cost dwarfs a 200x20 matmul (12332 us vs 4.4 us). Cap to one thread — but at **module level**, since a cap inside `main()` is invisible to spawned workers | measured | [thread-cap-placement-spawn](gotchas/thread-cap-placement-spawn.md) |
| I14 | pyro's chain progress bars are tqdm → **stderr**, which `Tee` (stdout only) never saw; and `Tee.flush()` is never called, so `run.log` is block-buffered and a hard kill can truncate it | measured | [log-streams-and-buffering](gotchas/log-streams-and-buffering.md) |
